/*
Solves problem 1213G from Codeforces
https://codeforces.com/contest/1213/problem/G
Using Disjoint Set Union as an utility for a Minimum Spanning Tree
*/
#include <bits/stdc++.h>
//#pragma GCC optimize ("O3")
//#pragma GCC target ("sse4")
//#pragma GCC target ("avx,tune=native")
//Use above if bruteforcing with lots of small operations. Or just use it anytime, there's no downside.
//AVX is better slightly
using namespace std;
typedef long long ll;
typedef pair <int, int> pii;
typedef pair <ll, ll> pll;
typedef long double ld;

ll inv(ll a, ll b){
    return (a > 1 ? b - inv(b%a, a)*b/a : 1);
}

#define rc(s) return cout << s, 0
#define fi first
#define se second
#define L (nod << 1)
#define R (L | 1)
#define smax(X, Y) ((X) > (Y) ? (X) : (Y))
#define smin(X, Y) ((X) < (Y) ? (X) : (Y))
#define N 200010

const ld e = 2.71828182845904523536028747;
const ld PI = 3.14159265359;
const ll inf = (1LL<<62);
const int iinf = (1<<30);
//CLOCKS_PER_SEC

struct lol{
    int a, b, c;
    bool operator<(const lol &aux)const{
        return c < aux.c;
    }
} M[N];
int n, m, idx = 1, p[N];
pii q[N];
ll cnt, ans[N], ct[N];

int fnd(int q){
    if (q == p[q]) return q;
    p[q] = fnd(p[q]);
    return p[q];
}

int main(){
    //mt19937 rng(chrono::steady_clock::now().time_since_epoch().count());
    ios_base::sync_with_stdio(0); cin.tie(0);
    //ifstream cin ("test.in");
    cin >> n >> m;
    for (int i=1; i<n; i++) cin >> M[i].a >> M[i].b >> M[i].c;
    for (int i=1; i<=m; i++) cin >> q[i].fi, q[i].se = i;
    sort(M+1, M+n);
    sort(q+1, q+1+m);
    for (int i=1; i<=n; i++) p[i] = i, ct[i] = 1;
    for (int i=1; i<=m; i++){
        while (idx < n && M[idx].c <= q[i].fi){
            int a = fnd(M[idx].a), b = fnd(M[idx].b);
            cnt += ct[a] * ct[b];
            //cout << i << ' ' << idx << ' ' << cnt << ' ' << a << ' ' << b << ' ' << ct[a] << ' ' << ct[b] << '\n';
            ct[b] += ct[a];
            p[a] = b;
            idx++;
        }
        ans[q[i].se] = cnt;
    }
    for (int i=1; i<=m; i++) cout << ans[i] << ' ';
    return 0;
}
